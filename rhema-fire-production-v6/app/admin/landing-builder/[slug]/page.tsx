...
export default function LandingBuilder({ params }: { params: { slug: string } }) {
  ...
  useEffect(() => {
    fetch(`/api/admin/landing-pages/${slug}`)
      .then(r => r.json())
      .then(d => {
        setTitle(d.title || "");
  ...
  async function save() {
    ...
    const res = await fetch(`/api/admin/landing-pages/${slug}`, {
      ...
      body: JSON.stringify({ title, contentJson: json }),
    });
  ...
  return (
    <AdminShellClient title={`Landing Builder: ${slug}`}>
      <div style={{display:"grid", gridTemplateColumns:"1fr 1fr", gap:16}}>
        <div>
          ...
          <div style={{display:"grid", gap:10}}>
            {sections.map((s, i) => (
              <div key={i} className="rf-card rf-card--tight">
                ...
                <div className="rf-card__meta" style={{marginTop:6}}>Edit JSON below if you want fine control.</div>
              </div>
            ))}
          </div>

          <div className="rf-field" style={{marginTop:12}}>
            <label className="rf-label">Builder JSON</label>
          ...
          <div style={{display:"flex", gap:12, alignItems:"center"}}>
            ...
            <a className="rf-btn rf-btn--secondary" href={`/lp/${slug}`} target="_blank" rel="noreferrer">Open Live</a>
          </div>
        </div>
